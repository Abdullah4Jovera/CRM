...

{
 ...
 days I have&nbsp; {leave_status} your leave request for {leave_reason}.</span><br></p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;"><span style="font-family: var(--bs-body-font-family); font-weight: var(--bs-body-font-weight); text-align: var(--bs-body-text-align);">We request you to complete all your pending work or any other important issue so that the company does not face any any loss or problem during your absence. We appreciate your thoughtfulness to inform us well in advance.</span></p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Feel free to reach out if you have any questions.</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Thank You,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Regards,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">HR Department,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">{app_name}</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">{app_url}</p><p></p>',
    ...
    public function getincExpBarChartDataLeadContractDeal()
    {
        $month[]          = __('January');
        $month[]          = __('February');
        $month[]          = __('March');
        ...
        for($i = 1; $i <= 12; $i++)
        {
            ...
            $leads[] = Lead::join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->whereNull('is_converted')->where('created_by', '=', $this->creatorId())->whereRaw('year(`date`) = ?', array(date('Y')))->whereRaw('month(`date`) = ?', $i)->count();
            $contracts[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->where('contract_stage','!=','cm_signed')->where('leads.created_by', '=', $this->creatorId())->whereRaw('year(leads.date) = ?', array(date('Y')))->whereRaw('month(leads.date) = ?', $i)->count();
            $deals[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->where('contract_stage','cm_signed')->where('leads.created_by', '=', $this->creatorId())->whereRaw('year(leads.date) = ?', array(date('Y')))->whereRaw('month(leads.date) = ?', $i)->count();
            $clients[] = Lead::join('user_leads','leads.id','=','user_leads.lead_id')->join('clients' , 'leads.client_id', '=' , 'clients.id')->where('user_leads.user_id','=',\Auth::user()->id)->where('leads.created_by', '=', $this->creatorId())->whereRaw('year(leads.date) = ?', array(date('Y')))->whereRaw('month(leads.date) = ?', $i)->count();
            // $clients[] = Client::whereRaw('year(`created_at`) = ?', array(date('Y')))->whereRaw('month(`created_at`) = ?', $i)->count();


        }
        // dd($clients);
        $dataArr['leads']  = $leads;
        $dataArr['contracts'] = $contracts;
        $dataArr['deals'] = $deals;
        $dataArr['clients'] = $clients;
        return $dataArr;
    }
