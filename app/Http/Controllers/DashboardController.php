...
class DashboardController extends Controller
{
    ...
    public function crm_dashboard_index()
    {
        ...
        if(\Auth::user()->can('show crm dashboard'))
        {
            ...
            {
                // dd(\Auth::user()->getincExpBarChartDataLeadContractDeal());
                $crm_data = [];

                $leads = Lead::where('created_by', \Auth::user()->creatorId())->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->whereNull('is_converted')->get();
                $deals = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->where('contract_stage','cm_signed')->get();
                $contracts = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->where('contract_stage','!=','cm_signed')->get();
                $incExpBarChartData  = \Auth::user()->getincExpBarChartDataLeadContractDeal();
                $crm_data['currentYear'] = date('Y');
                $clients = Lead::where('leads.created_by', \Auth::user()->creatorId())->join('clients' , 'leads.client_id', '=' , 'clients.id')->join('user_leads','leads.id','=','user_leads.lead_id')->where('user_leads.user_id','=',\Auth::user()->id)->get();
                $crm_data['total_leads']= count($leads);
                $crm_data['total_deals']= count($deals);
                $crm_data['total_contracts']= count($contracts);
                $crm_data['total_clients']= count($clients);
                return view('dashboard.crm-dashboard', compact('crm_data','incExpBarChartData'));
            }
        }
        else
        {
    ...
    public function getNotif($id){
        ...
        $data = [];
        $data_get = UserNotifications::where('user_id',\Auth::user()->id)->whereBetween('created_at', [
            Carbon::parse('last monday')->startOfDay(),
            Carbon::parse('next friday')->endOfDay(),
        ])->whereNull('is_read')->orderBy('created_at','DESC')->with('activity')->get();
        $click = 1;
        foreach ($data_get as $key ) {
            if (!empty($key->activity->log_type)) {
                $data_html[] ="<a href=".url('data_update/'.$key->id.'/'.$click.'/'.$key->activity->deal_id.'')." class='dropdown-item '>
                    <span class='text-primary'>". $key->activity->log_type." <br />
                        <span style='color:#645f5f90;'>" .$key->activity->getRemark(). "</span>
                        <br />
                        <span style='color:#645f5f90;'>" .$key->created_at. "</span>
                    </span>
                </a>";
            }

        }
        $data['get_data'] =  $data_get;
        $data['data_html'] =  $data_html;
        return $data;

    }

    public function dataUpdate($id,$click,$deal_id){
        UserNotifications::where('id',$id)->update(['is_read'=>$click]);
        $deal = Deal::where('id',$deal_id)->first();
        if ($deal->contract_stage == 'cm_signed'){
            return redirect()->route('deals.show',$deal->id);
        }else{
            return redirect()->route('contract.show',$deal->id);
        }
