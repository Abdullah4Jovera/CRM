...
class DealController extends Controller
{
    ...
    public function index()
    {
        ...
        if($usr->can('manage deal'))
        {
            ...
            $cnt_deal                = [];
            $cnt_deal['total']       = number_format($deals);
            $cnt_deal['this_month']  = number_format($collection);
            $cnt_deal['this_week']   = number_format($collection_month);
            $cnt_deal['last_30days'] = number_format($curr_month);
...
//                    dd($e);
    ...
    public function getPipeline($id){
        ...
        // $pipleineDeals = Deal::leftJoin('leads', 'deals.id','=', 'leads.is_converted')->leftJoin('user_leads' ,'leads.id','=','user_leads.lead_id')->where('deals.pipeline_id',$id)->where('user_leads.user_id',\Auth::user()->id)->toSql();
        // dd($pipelineDeals);
