...
class ApiController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($attr) {
            $clinet = Client::where('phone',$request->phone)->first();
            $dbPass = Crypt::decryptString($clinet->password);
            if ($dbPass == $request->password) {
                return $this->success([
                    'token' => $clinet->createToken('API Token')->plainTextToken,
                    'user'=> $clinet->id,
        //            'user'=> auth()->user()->id,
                    // 'settings' =>$settings,
                ],'Login successfully.');
...
//         ],'Login successfully.');
    ...
    public function register(Request $request){
        ...
        if ($attr) {
            ...
            $clinet = Client::firstOrCreate([
                'phone'=>$request->phone,
            ], [
                'name'=>$request->name,
                'password'=>$password,
            ]);
            if ($clinet->wasRecentlyCreated) {
                return $this->success([
                    // 'token' => auth()->user()->createToken('API Token')->plainTextToken,
                    'user'=> $clinet->id,
                ],'Successfully Register.');
            } else {
                return $this->error('User already exist', 401);
            }
        }else {
            return $this->error('Getting some Error', 404);
        }
        return $attr;
