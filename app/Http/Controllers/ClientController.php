...
class ClientController extends Controller
{
    ...
    public function index()
    {
        ...
        // {
            ...
            $userId = Auth::id();
            $pipelineId = Auth::user()->default_pipeline;
            $clients = Lead::join('clients', 'leads.client_id', '=', 'clients.id')
            ->join('user_leads', 'leads.id', '=', 'user_leads.lead_id')
            ->join('users', 'user_leads.user_id', '=', 'users.id')
            // ->join('pipelines', 'leads.pipeline_id', '=', 'pipelines.id')
            // ->join('stages', 'leads.stage_id', '=', 'stages.id')
            ->select('leads.*', 'clients.name as client_name','clients.phone as client_phone','clients.email as client_email',)
            ->where('users.id', $userId)
            // ->where('pipeline_id', '!=', 10)
            // ->whereNull('leads.is_reject')
            ->whereNull('leads.is_converted')
