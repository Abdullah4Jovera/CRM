...
//            $department->prepend('Select Department', '');
    ...
    public function leadreport(Request $request)
    {
        ...
        $leads = Lead::orderBy('id')->where('products',1)->whereNull('is_converted')->where('date', '>=', date('Y-m-01', $start))->where('date', '<=', date('Y-m-t', $end))->where('created_by', \Auth::user()->creatorId())->count();
        ...
        while ($currentdate <= $end) {
            ...
            if (!empty($request->start_month)) {
                $leadFilter = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',1)->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->get();
                $Bb[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',1)->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();
                $Pl[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',2)->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();
                $Ml[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',3)->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();
                $Marketing[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Marketing')->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();
                $TeleSales[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Tele Sales')->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();
                $Other[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Other')->whereNull('leads.is_converted')->whereMonth('leads.date', $request->start_month)->whereYear('leads.date', $year)->count();

            } else {
                $leadFilter = Lead::where('created_by', \Auth::user()->creatorId())->where('products',1)->whereNull('is_converted')->whereMonth('date', $month)->whereYear('date', $year)->get();
                $Bb[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',1)->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
                $Pl[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',2)->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
                $Ml[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.products',3)->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
                $Marketing[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Marketing')->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
                $TeleSales[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Tele Sales')->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
                $Other[] = Lead::join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.lead_type','Other')->whereNull('leads.is_converted')->whereMonth('date', $month)->whereYear('date', $year)->count();
            }
        ...
        $lead_pipeline = Pipeline::where('created_by', 2)->get();

        $leadpipelineName = [];
        $leadpipelineeData = [];
        foreach ($lead_pipeline as $lead_pipeline_data) {
            $lead_pipeline = lead::where('created_by', \Auth::user()->id)->where('pipeline_id', $lead_pipeline_data->id)->whereNull('is_converted')->count();
            $leadpipelineName[] = $lead_pipeline_data->name;
            $leadpipelineeData[] = $lead_pipeline;
        }

        return view('report.lead', compact('devicearray', 'leadsourceName', 'leadsourceeData', 'labels', 'data','products', 'filter', 'monthList','leads', 'user', 'leadpipelineName', 'leadpipelineeData'));
    }
    ...
    public function dealreport(Request $request)
    {
        ...
        while ($currentdate <= $end) {
            ...
            if (!empty($request->start_month)) {
                $leadFilter = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',1)->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->get();
                $Bb[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',1)->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();
                $Pl[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',2)->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();
                $Ml[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',3)->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();
                $Marketing[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Marketing')->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();
                $TeleSales[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Tele Sales')->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();
                $Other[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Other')->whereMonth('deals.date', $request->start_month)->whereYear('deals.date', $year)->count();

            } else {
                $leadFilter = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',1)->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->get();
                $Bb[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',1)->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
                $Pl[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',2)->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
                $Ml[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.products',3)->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
                $Marketing[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Marketing')->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
                $TeleSales[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Tele Sales')->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
                $Other[] = Deal::join('leads','deals.id','=','leads.is_converted')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('deals.lead_type','Other')->whereMonth('deals.date', $month)->whereYear('deals.date', $year)->count();
            }
        ...
        $leadpipelineeData = [];
        foreach ($lead_pipeline as $lead_pipeline_data) {
            $Signatured[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',1)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $Submission[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',2)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $Approved[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',3)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $Rejected[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',4)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $Disbursed[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',5)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $Collected[] = Lead::join('deals','leads.is_converted','=','deals.id')->join('user_leads','leads.id', '=' ,'user_leads.lead_id' )->where('user_leads.user_id',\Auth::user()->id)->where('leads.deal_stage_id',6)->where('deals.contract_stage','cm_signed')->where('leads.pipeline_id', $lead_pipeline_data->id)->count();
            $leadpipelineName[] = $lead_pipeline_data->name;
        }

        $leadpipelineeData['Signatured'] = $Signatured;
        $leadpipelineeData['Submission'] = $Submission;
        $leadpipelineeData['Approved'] = $Approved;
        $leadpipelineeData['Rejected'] = $Rejected;
        $leadpipelineeData['Disbursed'] = $Disbursed;
        $leadpipelineeData['Collected'] = $Collected;
        // dd($leadpipelineeData);

        return view('report.deal', compact('devicearray', 'leadsourceName', 'leadsourceeData', 'labels', 'data','products', 'filter', 'monthList','leads', 'user', 'leadpipelineName', 'leadpipelineeData'));
    }
...
//        dd($request->all());
    ...
    public function reportcommission(Request $request)
    {
            ...
            if (!empty($request->start_month) ) {
                ...
                // dd($request->start_month);
                $year = date('Y');
            } else {
                $month = date('m');
                $year = date('Y');
            }
            $reportData['hod'] = ServiceCommission::select('service_commissions.id','users.name',
            DB::raw('SUM(service_commissions.without_vat_commission) as total'),
            DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.hodsale','=','users.id')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.hodsale = users.id  AND  ld.deal_stage_id = 6  GROUP BY users.id ) as totalAmountBySix'))
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ->get();


            $reportData['salemanager'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.salemanager','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.salemanager = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['coordinator'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.coordinator','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.coordinator = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['team_leader'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.team_leader','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.team_leader = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['salesagent'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.salesagent','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.salesagent = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['salemanagerref'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.salemanagerref','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.salemanagerref = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['agentref'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.agentref','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.agentref = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['ts_team_leader'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.ts_team_leader','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.ts_team_leader = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['tsagent'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.tsagent','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.tsagent = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            // ->where('deals.contract_stage','cm_signed')
            // ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.tsagent = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            // ->whereBesers.id', 'users.name')
            // ->get();

            $reportData['marketingmanager'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.marketingmanager','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.marketingmanager = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $reportData['marketingagent'] = ServiceCommission::select('service_commissions.id',DB::raw('SUM(service_commissions.without_vat_commission) as total'),'users.name',DB::raw('SUM(service_commissions.hodsalecommission) as total_commission'))
            ->join('users','service_commissions.marketingagent','=','users.id')
            ->join('deals','service_commissions.deal_id','=','deals.id')
            ->where('deals.contract_stage','cm_signed')
            ->addSelect(DB::raw('(select without_vat_commission from service_commissions Join deals as dl on service_commissions.deal_id = dl.id Join leads as ld on  dl.id = ld.is_converted where service_commissions.marketingagent = users.id  AND  ld.deal_stage_id = 6 GROUP BY users.id ) as totalAmountBySix'))
            ->join('leads','deals.id','=','leads.is_converted')
            ->where('deals.contract_stage','cm_signed')
            ->whereMonth('leads.collection_date', $month)->whereYear('leads.collection_date', $year)
            ->groupBy('users.id', 'users.name')
            ...
            $lead_id       = Lead::where('deal_stage_id',6)->pluck('is_converted');
            $collection       = ServiceCommission::whereIn('deal_id',$lead_id)->sum('without_vat_commission');
            $collection_month  = ServiceCommission::whereIn('deal_id',$lead_id)->whereMonth('created_at', '=', date('m'))->sum('without_vat_commission');
            ...
            $reject_month  = ServiceCommission::whereIn('deal_id',$lead_id_reject)->whereMonth('created_at', '=', date('m'))->sum('without_vat_commission');
            ...
            $curr_month  = ServiceCommission::whereIn('deal_id',$id)->whereMonth('created_at', '=', date('m'))->sum('without_vat_commission');
            ...
            return view('report.commision', compact('reportData', 'cnt_deal','month'));
