    ...
    @if ($pipeline->id == '1')
        <ul class="nav nav-tabs nav-justified mb-3" id="ex1" role="tablist">
            ...
            @foreach ($pipelineDeals as $pipelineDeal)
                @if ($pipelineDeal->id != 1)
                    <li class="nav-item" role="presentation">
                        <button class="nav-link getDealById {{ $lastPipelineId == null ? "active" : "" }}" data-id="{{$pipelineDeal->id}}" id="home-tab" data-bs-toggle="tab" data-bs-target="#{{str_replace(' ', '_', $pipelineDeal->name);}}" type="button" role="tab" aria-controls="{{$pipelineDeal->name}}" aria-selected="true">{{$pipelineDeal->name}}</button>
                    </li>
                    @php
                        $lastPipelineId = $pipelineDeal->id ;
                    @endphp
                @endif
            @endforeach
        </ul>
        ...
        @endphp
        <div class="tab-content" id="all-deals-content">
            @foreach ($pipelineDeals as $pipeliendeal)
                @if ($pipeliendeal->id != 1)
                    <div class="tab-pane fade {{ $last_pipeline_id == null ? "show active" : "" }}" id="{{str_replace(' ', '_', $pipeliendeal->name);}}" role="tabpanel" aria-labelledby="home-tab" tabindex="0">
                        <div class=" all-deals-content">
                            <div class="col-sm-12">
                            @php
                                $stages = $pipeliendeal->stages;
                    ...
                    @php
                        $last_pipeline_id = $pipeliendeal->id ;
                    @endphp
                @endif
            @endforeach
        </div>
