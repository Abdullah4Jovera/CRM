...
<div class="col-4 form-group">
    ...
    <select class="form-control select" name="business_banking_services" id="business_banking_services">
        <option selected disabled>Business Banking Services</option>
        <option {{($data->business_banking_services =="business_loan")?'selected':""}} value="business_loan">Business Loan</option>
        <option {{($data->business_banking_services =="fleet_finance")?'selected':""}} value="fleet_finance">Fleet Finance ( Auto Loans)</option>
        <option {{($data->business_banking_services =="lgcs")?'selected':""}} value="lgcs">LGs / LCs</option>
        <option {{($data->business_banking_services =="account_opening")?'selected':""}} value="account_opening">Account Opening</option>
    </select>
</div>
 <div class="col-4 form-group">
 ...
 <div class="col-4 form-group">
     <input type="number" value="{{$data->yearly_turnover}}" name="yearly_turnover" id="yearly_turnover" class="form-control" required  placeholder="Company Turnover in Last Year" >
 </div>
...
$(document).ready( function () {
    ...
    $(document).ready(function () {
        ...
        console.log(business_banking_services);
        if (business_banking_services =='business_loan') {
            var businessLoan = `@if ($data !=null)
                <div class="col-6 form-group">
                        <select class="form-control select" name="have_any_pos" id="have_any_pos">
                            ...
                            <option {{($data->have_any_pos =="yes")?'selected':""}} value="yes">Yes</option>
                            <option {{($data->have_any_pos =="no")?'selected':""}} value="no">No</option>
                        </select>
                    </div>
                    <div class="col-6 form-group">
                        {{ Form::text('monthly_amount', $data->monthly_amount, array('class' => 'form-control monthly_amount','required'=>'required', 'placeholder'=>'POS Turnover Monthly (Approx)','disabled'=> '')) }}
                    </div>
            @endif`;
            ...
            let fleetFinance = `@if ($data !=null)
                <div class="col-6 form-group">
                    ...
                    <div class="col-6 form-group">
                        {{ Form::text('monthly_emi', $data->monthly_emi, array('class' => 'form-control monthly_emi','required'=>'required', 'placeholder'=>'Total EMI Paid (Monthly)','disabled'=> '')) }}
                    </div>
            @endif`;
               $('.businessBankingServices').html(fleetFinance);
               $('#yearly_turnover').removeAttr('disabled','disabled');
        }else if (business_banking_services =='lgcs') {
            let lgcs = `@if ($data !=null)
                <div class="col-12 form-group">
                        <select class="form-control select" name="lgcs" id="lgcs">
                            ...
                            <option {{($data->have_any_pos =="no")?'selected':""}} value="Govt. Project">Govt. Project</option>
                            <option {{($data->have_any_pos =="no")?'selected':""}} value="Semi Govt. Project">Semi Govt. Project</option>
                            <option {{($data->have_any_pos =="no")?'selected':""}} value="Private Project">Private Project</option>
                            <option {{($data->have_any_pos =="no")?'selected':""}} value="National Housing Loan (NHL)">National Housing Loan (NHL)</option>
                        </select>
                    </div>
            @endif`;
    ...
    $(document).on('change','#business_banking_services', function () {
        ...
        if (business_banking_services =='business_loan') {
            var businessLoan = `<div class="col-6 form-group">
                ...
                <div class="col-6 form-group">
                    {{ Form::text('monthly_amount', null, array('class' => 'form-control monthly_amount', 'placeholder'=>'POS Turnover Monthly (Approx)','disabled'=> '')) }}
                </div>`;
                $('.businessBankingServices').html(businessLoan);
               $('#yearly_turnover').removeAttr('disabled','disabled');
        }else if (business_banking_services =='fleet_finance') {
            let fleetFinance = `<div class="col-6 form-group">
                ...
                <div class="col-6 form-group">
                    {{ Form::text('monthly_emi', null, array('class' => 'form-control monthly_emi', 'placeholder'=>'Total EMI Paid (Monthly)','disabled'=> '')) }}
                </div>`;
               $('.businessBankingServices').html(fleetFinance);
               $('#yearly_turnover').removeAttr('disabled','disabled');
        }else if (business_banking_services =='lgcs') {
