...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Rejected Leads')}}</li>
@endsection
{{-- @section('action-btn')
    <div class="float-end">
                                ...
                                @if(count($leads) > 0)
                                    ...
                                    {{-- @dd($lead) --}}
                                        @if (!empty($lead->is_reject))
                                            <tr>
                                                <td>{{ $i  }}</td>
                                                <td>{{ optional($lead)->client_name  }}</td>
                                                <td>{{ optional($lead)->client_phone  }}</td>
                                                <td>{{  optional($lead)->pipeline_name }}</td>
                                                <td>{!! $lead->notes !!}</td>
                                                {{-- <td>
                                                    @foreach($lead->users as $user)
                                                        @if($lead->created_by != $user->id && $user->designation != 'Jovera' && $user->type != 'company')
                                                         ...
                                                         <a href="#" class="btn btn-sm mr-1 p-0 rounded-circle">
                                                             <img loading="lazy" alt="image" data-toggle="tooltip" data-original-title="{{$user->name}}" @if($user->avatar) src="{{asset('/storage/uploads/avatar/'.$user->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif class="rounded-circle " width="25" height="25">
                                                         </a>
                                                       @endif
                                                    @endforeach
                                                </td> --}}
                                                        ...
                                                        <span>
                                                            ...
                                                            @can('edit lead')
                                                                <div class="action-btn bg-info ms-2">
                                                                    <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-url="{{ route('leads.transfer',$lead->id) }}" data-ajax-popup="true" data-size="xl" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Lead Edit')}}">
