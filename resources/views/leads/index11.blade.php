...
@push('script-page')
    ...
    <script>
        ...
        function getSearchByUser() {
            ...
            let pipeline = $('#pipeline_id').val();
            let datetimes = $('#datetimes').val();
            $.ajax({
                type: "POST",
                url: "{{ URL::to('leads/filterSearch') }}",
                data: {
                        user : user
                        pipeline : pipeline
                        datetimes : datetimes
        ...
        $(document).on("change", "#search_lead",function () {
            let searchVal= $('#search_lead').val();
            if (searchVal != null) {
                let pipelineId = $("#default_pipeline_id").val();
                $.ajax({
                    type: "POST",
...
@section('action-btn')
    <div class="float-end d-flex">
        ...
        <select name="search_lead" id="search_lead" class="form-control select2">
            <option value="" selected>Search Lead </option>
            @foreach ($allLeads as $al)
                <option value="{{$al->name}}">{{$al->name}} - {{$al->phone}} </option>
            @endforeach

        </select>

        {{-- <input type="search" name="search_lead" id="search_lead" class ='form-control select' placeholder="Search"> --}}

        <a href="{{ route('leads.list') }}" data-size="lg" data-bs-toggle="tooltip" title="{{__('List View')}}" class="btn btn-sm btn-primary" style="margin-bottom: 15px !important; margin-right: 5px !important;">
            <i class="ti ti-list"></i>
        </a>
        <a href="#" data-url="{{ route('leads.filter') }}" data-size="lg" data-ajax-popup="true" title="{{__('Filter')}}" data-bs-toggle="tooltip" class="btn btn-sm btn-primary" style="margin-bottom: 15px !important; margin-right: 5px !important;">
            <i class="ti ti-filter"></i>
        </a>
        <a href="#" data-size="lg" style="margin-bottom: 15px !important;" data-url="{{ route('leads.create') }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Create New Lead')}}" class="btn btn-sm btn-primary">
        ...
        <div class="row mainPage" >
            <div class="col-sm-12">
                @foreach ($pipelineLeads as $pipelineLead)
                    @if ($pipelineLead->id != 1)
                            @php
                                $lead_stages = $pipelineLead->leadStages;
                                $json = [];
